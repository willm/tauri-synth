...
mod tests {
  ...
  fn test_envelope() {
    ...
    let plottable_data = data
      .iter()
      .enumerate()
      .map(|x: (usize, &f64)| -> (f64, f64) { (x.0 as f64, *x.1) })
      .collect();
    ...
    Page::single(&ContinuousView::new().add(l1))
      .save("./plots/enveloppe.svg")
      .unwrap();
  }
